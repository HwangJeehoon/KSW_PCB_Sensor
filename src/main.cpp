...
static constexpr uint32_t TX_INTERVAL_US  = 2000;  // 500 Hz = 2000 µs; 0 = full speed

// Serial TX 버퍼 여유 확인에 사용할 프레임 크기
#if   DEBUG_TEXT == 0
static constexpr uint8_t FRAME_BYTES = 25;
#elif DEBUG_TEXT == 2
static constexpr uint8_t FRAME_BYTES = 21;
#else
static constexpr uint8_t FRAME_BYTES = 0;  // 텍스트 모드: 버퍼 체크 생략
#endif
...
void loop() {
    ...
    // --- Serial TX ---
    // last_tx_us += TX_INTERVAL_US: loop 내 지연이 누적되지 않는 drift-free 캐던스.
    // TX_INTERVAL_US == 0이면 조건이 항상 참 → full speed.
    static uint32_t last_tx_us = 0;
    const uint32_t t_us = micros();
    if (TX_INTERVAL_US == 0 || t_us - last_tx_us >= TX_INTERVAL_US) {
        last_tx_us += TX_INTERVAL_US;

        // TX 시도마다 seq 증가: 실제 송신 여부와 무관하게 증가하므로
        // 수신 측에서 seq 불연속으로 드롭된 프레임을 감지할 수 있음.
        const uint16_t seq = g_seq++;

#if   DEBUG_TEXT == 1
        sendDebugFrame(t_us, seq,
                       enc_latest[0], enc_latest[1],
                       lc_latest[0],  lc_latest[1]);
#elif DEBUG_TEXT == 2
        if (FRAME_BYTES == 0 || Serial.availableForWrite() >= FRAME_BYTES) {
            sendBinaryFrame21(seq,
                              enc_latest[0], enc_latest[1],
                              lc_latest[0],  lc_latest[1]);
        }
#else  // 0: 25-byte (기본)
        if (FRAME_BYTES == 0 || Serial.availableForWrite() >= FRAME_BYTES) {
            sendBinaryFrame25(t_us, seq,
                              enc_latest[0], enc_latest[1],
                              lc_latest[0],  lc_latest[1]);
        }
#endif
